...
_Generated: 2026-03-30 12:23 UTC_
...
### Quality Finding: CLOD price extraction on Costco receipts

The explicit prompt rule (`price` = right-hand price column) did not resolve CLOD's Costco price errors. CLOD (`Qwen/Qwen2.5-7B-Instruct-Turbo`) is still extracting per-unit rates and subtotals (e.g. `1.20`, `0.60`, `160.70`) rather than the line-item total. Costco receipts use a multi-column format (`qty × unit_price = line_total`) that requires the model to parse column relationships — a task Qwen 2.5 7B handles unreliably. OpenRouter (`claude-haiku-4.5`) handles this correctly because it is a stronger model. This is a model capability gap, not a prompt issue, and represents a real quality difference between the two providers on structured multi-column receipts.
