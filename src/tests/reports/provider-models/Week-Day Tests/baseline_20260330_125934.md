...
_Generated: 2026-03-30 12:59 UTC_
...
_openrouter costs are token-based estimates computed from the published rate card and can be independently audited against the per-receipt token counts above. clod costs are taken directly from the API response (`llm_cost_source: api`); the per-row figures don't follow a single predictable per-token rate (lower-output receipts imply a different rate than higher-output ones like Costco and T&T), so individual clod figures cannot be independently verified without the provider's full rate structure._
